...
public class ID3
{
	...
	public void printTree(TreeNode node, String tab) {
		...
		for (int i=0; i < numvalues; i++) {
			//if (node.children[i] != null && getAllValues(node.children[i].data, outputattr ).length == 0) { System.out.println("// EMPTY"); } 
			System.out.println(
				tab + "if( " + attributeNames[node.decompositionAttribute] + ".equalsIgnoreCase(\"" + 
	...
	public static void main(String[] args) throws Exception {
		ID3 me = new ID3();
		int status = me.readData("adult.data.modified_training.data");
		if (status <= 0) return;
		me.createDecisionTree();
	}
}
//- See more at: http://www.codemiles.com/java/java-code-for-decision-tree-algorithm-t5747.html#sthash.0nc2KSsY.dpuf